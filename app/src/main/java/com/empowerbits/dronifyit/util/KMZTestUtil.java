...
import com.empowerbits.dronifyit.util.wpml.WaypointInfoModel;
 ...
 * @description: KMZ Test Utility for DJI SDK V5
 */
public class KMZTestUtil {
    ...
    public static WaylineMission createWaylineMission(){
        ...
        waylineMission.setCreateTime(((Long)timestamp).doubleValue());
        waylineMission.setUpdateTime(((Long)timestamp).doubleValue());
    ...
    public static WaylineMissionConfig createMissionConfig(MissionSetting missionSetting){
        ...
        payloadInfo.setPayloadType(WaylinePayloadType.PAYLOAD_CAMERA);  // Generic camera (65534)
        payloadInfo.setPayloadPositionIndex(0);  // ✅ MUST be 0 (main camera position)
        payloadInfo.setIsPayloadPositionIndexSet(true);  // ✅ Explicitly mark as set
    ...
    public static WaylineActionInfo transTakePhoto() {
        ...
        photoTypes.add(CameraLensType.WIDE); // Add WIDE lens type
